<template>
  <a-form v-if="config" :model="config" auto-label-width>
    ...
    <!-- 本地文件系统配置 -->
    <a-card
      :title="$t('pages.settings.upload.LocalFileSystem')"
      class="upload-settings-card"
    >
      <a-form-item
        field="local.uploadPath"
        :label="$t('pages.settings.upload.uploadPath')"
      >
        <a-input
          v-model="config.local.uploadPath"
          :placeholder="$t('pages.settings.upload.placeholder.uploadPath')"
        />
      </a-form-item>

      <a-form-item
        field="local.maxSizeMB"
        :label="$t('pages.settings.upload.maxSizeMB')"
      >
        <a-input-number
          v-model="config.local.maxSizeMB"
          :min="1"
          :max="100"
          :placeholder="$t('pages.settings.upload.placeholder.maxSizeMB')"
        />
      </a-form-item>
    </a-card>
