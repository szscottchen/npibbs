...
### 安装：
- 配置<本地源代码目录>中的几个关键文件： server\bbs-go.yaml、site\env.production、、admin\.env.production。配置内容参考原文件中解释。
- 编译源代码，命令为go build -o <正式使用的程序名> main.go。
- <本地源代码目录>的site目录下执行：pnpm build
- <本地源代码目录>的admin目录下执行：pnpm build
- Mydql中新建项目数据库:create database <项目数据库>; 赋予grant all privileges on <项目数据库>.* to '<数据库管理员>'@'localhost'
- 在企业微信后台配置应用回调域名、可信域名、企业可信IP等，具体参考企业微信文档。
- 把<正式使用的程序名>和bbs-go.yaml拷贝到<项目运行根目录>
- 把<本地源代码目录>\server目录下的locales和migrations两个目录拷贝到<项目运行根目录>
- 把<本地源代码目录>\site目录下的\dist目录下所有内容拷贝到<项目运行根目录>\site
- 把<本地源代码目录>\admin目录下的\dist目录下所有内容拷贝到<项目运行根目录>\admin
- <项目运行根目录>下建两个空目录\temp和\uploads

如涉及nginx和docker配置参考相关系统文档。

### 启动服务
首次登录，用浏览器端访问：http://<服务器地址和端口号>/install。完成数据库和管理员的初始化。
