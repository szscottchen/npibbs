...
func ReadConfig() (cfg *Config, exists bool, err error) {
	...
	if exists {
		...
		// default config
		// 在ReadConfig函数的默认配置部分（约第127行）
		cfg = &Config{
			Language:       DefaultLanguage,
			Port:           8082,
			Installed:      false,
			AllowedOrigins: []string{"*"}, // 添加这一行
			Logger: LoggerConfig{
...
func getConfigFilePath(configName string) string {
	// 在开发环境（使用 go run）时，使用当前工作目录
	// 在生产环境（编译后运行）时，使用可执行文件所在目录
	...
	if err == nil {
		...
		if _, statErr := os.Stat(configPath); statErr == nil {
			slog.Info("Using config file from working directory", slog.String("path", configPath))
			return configPath
		}
	}

	// 如果当前目录没有，使用可执行文件所在目录
	exePath, err := os.Executable()
	if err != nil {
		slog.Error("Failed to get executable path", slog.Any("error", err))
		// 降级到当前工作目录
		if workDir != "" {
			return filepath.Join(workDir, configName)
		}
		return ""
	}

	configPath := filepath.Join(filepath.Dir(exePath), configName)
	slog.Info("Using config file from executable directory", slog.String("path", configPath))
	return configPath
}
