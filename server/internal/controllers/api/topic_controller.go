...
func (c *TopicController) BeforeActivation(b mvc.BeforeActivation) {
	b.Handle("GET", "/recommend", "GetRecommend")
	b.Handle("GET", "/need_a_hand", "GetNeedAHand")
}
...
func (c *TopicController) _GetBuiltInNodes() []models.NodeResponse {
	if len(c._BuiltInNodes) == 0 {
		c._BuiltInNodes = []models.NodeResponse{
			...
			{
				Id:   -3,
				Name: locales.Get("topic.node.need_a_hand"),
			},
		}
	}
...
func (c *TopicController) PostEditBy(topicId int64) *web.JsonResult {
	...
	var (
		...
		needAHand   = uint8(params.FormValueIntDefault(c.Ctx, "needAHand", 0))
	)

	err := services.TopicService.Edit(topicId, nodeId, tags, title, content, hideContent, needAHand)
	if err != nil {
		return web.JsonError(err)
	}
	return web.JsonData(render.BuildSimpleTopic(topic))
...
// 呼叫支援话题列表
func (c *TopicController) GetNeedAHand() *web.JsonResult {
	topics := cache.TopicCache.GetNeedAHandTopics()
	if len(topics) == 0 {
		return web.JsonCursorData([]interface{}{}, "", false)
	} else {
		return web.JsonCursorData(render.BuildSimpleTopics(topics, nil), "", false)
	}
}
