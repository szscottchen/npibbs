...
func (s *articleService) Publish(userId int64, form models.CreateArticleForm) (article *models.Article, err error) {
	...
	err = sqls.DB().Transaction(func(tx *gorm.DB) error {
		tagIds, txErr := repositories.TagRepository.GetOrCreates(tx, form.Tags)
		if txErr != nil {
			return txErr
		}
		if txErr = repositories.ArticleRepository.Create(tx, article); txErr != nil {
			return txErr
		}
...
func (s *articleService) Edit(articleId int64, tags []string, title, content string, cover *models.ImageDTO) error {
	...
	txErr := sqls.DB().Transaction(func(tx *gorm.DB) error {
		...
		if err := repositories.ArticleRepository.Updates(sqls.DB(), articleId, updates); err != nil {
			return err
		}
