...
// 更新
func (s *topicService) Edit(topicId, nodeId int64, tags []string, title, content, hideContent string, needAHand uint8) error {
	if len(title) == 0 {
...
func (s *topicService) _GetNodeTopics(nodeId, cursor int64, limit int) (topics []models.Topic, nextCursor int64, hasMore bool) {
	...
	if nodeId == constants.NodeIdNeedAHand {
		cnd.Eq("need_a_hand", 1)
	}
	if cursor > 0 {
