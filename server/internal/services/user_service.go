...
// IncrScoreForValue 价值评价获积分
func (s *userService) IncrScoreForValue(userId int64, score int, sourceId, description string) error {
	if score <= 0 {
		return errors.New("价值积分必须为正数")
	}
	err := s.addScore(userId, score, constants.ScoreSourceValue, sourceId, description)
	if err != nil {
		slog.Error(err.Error(), slog.Any("err", err))
		return err
	}
