...
// HasValueScore 检查是否已经对某评论进行过价值评价
func (s *userScoreLogService) HasValueScore(userId int64, sourceId string) bool {
	return repositories.UserScoreLogRepository.Count(sqls.DB(), sqls.NewCnd().
		Eq("user_id", userId).
		Eq("source_type", constants.ScoreSourceValue).
		Eq("source_id", sourceId)) > 0
}
